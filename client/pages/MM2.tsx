  ...
  return (
    <div>
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 -z-10 bg-gradient-to-b from-violet-900 via-purple-950 to-black" />
        <div className="container py-10 md:py-14">
          <div className="flex items-center justify-between">
            <span className="inline-block rounded-lg bg-violet-900/70 px-3 py-1.5 text-sm font-bold uppercase tracking-widest text-violet-100 ring-1 ring-violet-700/40">Murder Mystery 2</span>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="w-full md:w-80">
              <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search" className="bg-violet-950/40 border-violet-800/50" />
            </div>
          </div>

          {showHint && (
            <div className="mt-4 rounded-lg border border-violet-700/40 bg-violet-900/40 p-3 text-sm text-violet-100 flex items-center justify-between">
          ...
          <div className="mt-6">
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {filtered.map((it, idx) => {
                ...
                return (
                  <div key={it.id} className="group relative overflow-hidden rounded-2xl border border-violet-700/30 bg-violet-900/20 p-3 transition-transform duration-500 ease-out hover:border-violet-500/40 hover:shadow-lg hover:shadow-violet-900/20 hover:-translate-y-0.5 hover:scale-[1.02] animate-in fade-in-0 slide-in-from-bottom-2" style={{ animationDelay: `${(idx % 12) * 40}ms` }}>
                    <div className="relative aspect-square overflow-hidden rounded-xl ring-1 ring-violet-700/20 group-hover:ring-indigo-500/30 transition">
                      ...
                      <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-70">
                        <div className="absolute -inset-10 bg-[radial-gradient(circle_at_30%_20%,rgba(167,139,250,0.35),transparent_45%),radial-gradient(circle_at_80%_70%,rgba(99,102,241,0.3),transparent_40%)]" />
                      </div>
                      ...
                      <div className="absolute inset-x-2 bottom-2 translate-y-8 opacity-0 will-change-transform transition-all duration-500 ease-out group-hover:translate-y-0 group-hover:opacity-100">
                        <Button disabled={soldOut} onClick={() => addToCart(it)} className="w-full h-9 bg-gradient-to-r from-violet-500 to-indigo-500 hover:from-violet-600 hover:to-indigo-600 disabled:opacity-60">{soldOut ? "Sold Out" : "Add to Cart"}</Button>
                      </div>
                    </div>
                    <div className="mt-3">
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm font-semibold">{it.name}</h3>
                        <span className="rounded-md bg-violet-900/60 px-2 py-0.5 text-[11px] ring-1 ring-violet-700/40">{priceFmt.format(toLocalRounded(it.priceUSD))}</span>
                      </div>
                      <p className="mt-1 text-xs text-violet-200/80">Limited • Instant delivery</p>
                    </div>
                  </div>
                );
              })}
        ...
        <Popover open={cartOpen} onOpenChange={setCartOpen}>
          ...
          <PopoverContent className="w-80">
            <div className="space-y-3">
              ...
              <Button disabled={groupedCart.length === 0} onClick={() => { if (groupedCart.length === 0) return; setCartOpen(false); setCheckoutOpen(true); }} className="w-full bg-gradient-to-r from-violet-500 to-indigo-500 hover:from-violet-600 hover:to-indigo-600 disabled:opacity-60">Go to Checkout</Button>
            </div>
          </PopoverContent>
        </Popover>
        <CheckoutDialog
